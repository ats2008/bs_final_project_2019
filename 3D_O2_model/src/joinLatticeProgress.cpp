...
int main(int argc,char ** argv)
{
	if(argc>1)
		...
		{
			cout<<"\n plz enter the *source_path , destn path , destn_folder_name\n";
			cout<<" destn of files = destn_path/destn_folder_name/\n";
			cout<<" 2 and 3 IF not set, destn of files = source_path/compiled/\n";
			return 0;
		}
	if(argc>3)
		{
...
//		if (i>1) break;
		cout<<i<<" / "<<fnames.size()<<"\n";
		cout<<"READING PartHfnames file for \t:"<< i <<" : "<<BASE_FOLDER+"PartHfnames_"+*name+".txt"<<"\n";
		...
		alat.readFromBinary(BASE_FOLDER+*partname+".dat");
...
//			if(j>2) break;
			...
			blat.readFromBinary(BASE_FOLDER+*partname+".dat");
...
//			cout<<"cleared blat now \n";
		...
		cout<<"reading for 'lattice save' : "<<BASE_FOLDER+partFnames.back()+".dat"<<"\n";
		blat.readFromBinary(BASE_FOLDER+partFnames.back()+".dat");
		blat.clear_data();
